...
bool CSerialPort::ReadByte(BYTE &resp){
	...
	if (ReadFile(m_hComm, &rx, 1, &dwBytesTransferred, NULL)) //포트에 존재하는 데이터를 ReadFile을 통해 1바이트씩 읽어온다.
	{
...
bool CSerialPort::ReadBytes(BYTE* &resp, UINT size) {
	
	DWORD dwBytesTransferred = 0;

	if (ReadFile(m_hComm, &resp, size, &dwBytesTransferred, NULL))
	{
		cout << "test 2" << endl;
